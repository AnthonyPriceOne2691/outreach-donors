...
import { DONOR_STATUSES } from '../api/labels';
 ...
 *  без условий читается как «база пуста». */
...
/** Всего доноров по сводке сервера: она считается по всей базе, а не по фильтру. */
 ...
 * по-разному, и путать их нельзя: пуста база — зовут прогон; вердикта нет
 * во всей базе — сужать дальше бессмысленно; пусто только под сочетанием
 * условий — условия и названы.
 */
export function emptinessOf(
  filters: DonorFilters,
  counts: Record<string, number>,
): { title: string; detail: string; resettable: boolean } {
  const all = totalOf(counts);
  const resettable = isFiltered(filters);
  if (all === 0) {
    return {
      title: 'Доноров в базе нет.',
      detail: 'Их приносит прогон: выдача по ключам, затем пороги и судья.',
      resettable,
    };
  }
  if (filters.status !== null && (counts[filters.status] ?? 0) === 0) {
    return {
      ...
      detail: `Нет во всей базе, а не только под остальными условиями. Всего доноров — ${formatNumber(all)}.`,
      resettable,
    };
  }
  ...
  return {
    ...
    detail:
      conditions.length === 0
        ...
        : `Условия: ${conditions.join(', ')}. Всего доноров в базе — ${formatNumber(all)}.`,
    resettable,
  };
}
