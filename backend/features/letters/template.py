...
**Набор зон проверяется при разборе.** Шаблон без юридического блока
собирается так же легко, как правильный, и разница видна только по жалобе
на рассылку. Поэтому отсутствие зоны — отказ разбора, а не предупреждение.
...
class ZoneKind(StrEnum):
    ...
    REWRITE = "rewrite"
    #: В модель не уходит. Условия, подпись, юридический блок.
    FIXED = "fixed"
...
#: Без чего письмо нарушает закон почти во всех целевых странах.
...
REQUIRED_IN_ZONE: dict[str, tuple[str, ...]] = {
    "legal": ("postal_address", "unsubscribe_url"),
    ...
    for zone_name, required in REQUIRED_IN_ZONE.items():
        ...
        if absent:
            raise TemplateError(
                ...
                "Без них письмо нарушает закон почти во всех целевых странах"
            )
