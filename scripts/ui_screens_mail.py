...
Сюда же переехали экраны стоп-листа и рекламодателей со своей подготовкой:
экраны одного потока — один модуль, а в основном каталоге остаётся место
(после 25.09 он стоял на 498 строках из 500).
...
def mail_screens(norm: float, big: float) -> dict[str, dict[str, Any]]:
    ...
    return {
        "suppressions": {
            ...
            # заполняет его сам (`fill_target` ниже).
            "probes": [
...
def show_decided(page: Any) -> None:
